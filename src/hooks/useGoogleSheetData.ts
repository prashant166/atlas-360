// src/hooks/useGoogleSheetData.ts
...
export const useGoogleSheetData = () => {
  // simple placeholder so build passes
...
//           sampleInvalidRow: rows.find((row: any)=> !row.Province || !row.Geo || typeof row.Population !== 'number' || typeof row['Composite Score'] !== 'number')
//         });
